...
class FlipAdvisor:
    ...
    _ACTION_CATALOG: Dict[str, Dict[str, object]] = {
        "SpellDamage1": {"action_type": "essence", "action_name": "Essence spell", "base_cost": 30.0, "probability": 0.42, "value_delta": 34.0, "brick_risk": 0.22, "notes": "Força spell damage de forma relativamente controlada."},
        "CastSpeed1": {"action_type": "harvest_reforge", "action_name": "Harvest Reforge Speed", "base_cost": 20.0, "probability": 0.33, "value_delta": 28.0, "brick_risk": 0.28, "notes": "Busca cast speed preservando o upside do item."},
        "CritChanceSpells1": {"action_type": "bench_craft", "action_name": "Bench craft crit", "base_cost": 8.0, "probability": 1.0, "value_delta": 18.0, "brick_risk": 0.02, "notes": "Fechamento barato quando sobra espaço útil."},
        "Life1": {"action_type": "essence", "action_name": "Essence of Greed", "base_cost": 18.0, "probability": 0.48, "value_delta": 30.0, "brick_risk": 0.18, "notes": "Vida é o upgrade mais líquido para body armours e accessories."},
        "SpellSuppress1": {"action_type": "harvest_reforge", "action_name": "Harvest Reforge Defence", "base_cost": 24.0, "probability": 0.24, "value_delta": 42.0, "brick_risk": 0.30, "notes": "Suppression só entra em bases com componente de Evasion/Dex."},
    ...
    def recommend_plans(self, item_class: str = "", ilvl_min: int = 1, rarity: str = "rare", max_items: int = 30, min_profit: float = 0.0, min_listed_price: float = 0.0, anti_fix: bool = True, safe_buy: bool = False, stale_hours: float = 48.0, budget: float = 150.0, top_plans: int = 3) -> Tuple[List[CraftPlan], ScanStats]:
        opportunities, stats = self.scanner.scan_opportunities(item_class=item_class, ilvl_min=ilvl_min, rarity=rarity, max_items=max_items, min_profit=min_profit, min_listed_price=min_listed_price, anti_fix=anti_fix, safe_buy=safe_buy, stale_hours=stale_hours)
        plans = self.build_plans_from_opportunities(opportunities, budget=budget)
        return plans[:top_plans], stats

    def build_plans_from_opportunities(self, opportunities: List[ScanOpportunity], budget: float) -> List[CraftPlan]:
        plans: List[CraftPlan] = []
        market_index = self._build_market_index(opportunities)
        ranked = sorted(opportunities, key=lambda opp: (opp.score, opp.trusted_profit, opp.profit), reverse=True)
        for opportunity in ranked[:12]:
            plan = self._build_plan(opportunity, budget, market_index)
            if plan is not None:
                plans.append(plan)
        plans.sort(key=lambda plan: (plan.trusted_profit, plan.confidence_breakdown.overall_confidence, plan.expected_profit), reverse=True)
        for index, plan in enumerate(plans):
            plan.alternatives = [f"{alt.opportunity.base_type}: trusted {alt.trusted_profit:.1f}c" for alt in plans[index + 1 : index + 3]]
        return plans

    def _build_plan(self, opportunity: ScanOpportunity, budget: float, market_index: Dict[tuple[str, str, int, str], List[float]]) -> CraftPlan | None:
        ...
        exit_estimate = self._build_exit_estimate(opportunity, target, steps, market_index, budget, expected_craft_cost)
        ...
        if exit_estimate.required_structure.get("link_count", self._link_count(opportunity)) > self._link_count(opportunity):
            remaining_delta = max(exit_estimate.expected_sale_value - self._current_market_anchor(opportunity) - sum(step.expected_value_delta for step in steps), 0.0)
            linking_step = self._build_linking_step(opportunity, int(exit_estimate.required_structure["link_count"]), remaining_delta)
        ...
        expected_profit = round(exit_estimate.expected_sale_value - opportunity.listed_price - expected_craft_cost, 1)
        if expected_profit <= 0:
            return None
        confidence_breakdown = self._build_confidence_breakdown(opportunity, steps, exit_estimate, target, budget)
        ...
        if exit_estimate.required_structure.get("link_count", 0) > self._link_count(opportunity):
            risk_notes.append("requires_linking")
        if exit_estimate.evidence_strength == "weak":
            risk_notes.append("weak_exit_evidence")
        plan_explanation = self._build_plan_explanation(opportunity, target, steps, exit_estimate, confidence_breakdown)
        return CraftPlan(opportunity=opportunity, target=target, steps=steps, buy_cost=opportunity.listed_price, expected_craft_cost=expected_craft_cost, expected_sale_value=round(exit_estimate.expected_sale_value, 1), expected_profit=expected_profit, trusted_profit=trusted_profit, plan_confidence=round(confidence_breakdown.overall_confidence, 2), confidence_breakdown=confidence_breakdown, exit_estimate=exit_estimate, stop_condition=stop_condition, plan_explanation=plan_explanation, risk_notes=risk_notes)
    ...
    def _passes_stage_a(self, opportunity: ScanOpportunity, target: FlipTargetRecommendation, budget: float) -> Tuple[bool, List[str]]:
        ...
        if opportunity.item_family == "body_armour_defense":
            ...
            if self._link_count(opportunity) < 6 and self._six_link_cost_chaos() > budget and opportunity.market_median <= opportunity.listed_price:
                reasons.append("insufficient_budget_for_linking")
        return (not reasons, reasons)

    def _build_steps(self, opportunity: ScanOpportunity, target: FlipTargetRecommendation, missing_mods: List[str]) -> List[CraftActionEvaluation]:
    ...
    def _evaluate_action(self, opportunity: ScanOpportunity, mod: str) -> CraftActionEvaluation:
        config = self._ACTION_CATALOG.get(mod, {"action_type": "bench_craft", "action_name": "Bench stabilization", "base_cost": 6.0, "probability": 1.0, "value_delta": 10.0, "brick_risk": 0.02, "notes": "Fechamento conservador para preservar margem."})
        eligible = True
        failure_reason = ""
        ...
        elif mod in {"Resist1", "Attributes1", "CritChanceSpells1"} and opportunity.open_suffixes <= 0:
        ...
        expected_cost = round(float(config["base_cost"]) / max(probability, 0.05), 1) if eligible else 0.0
        expected_value_delta = round(float(config["value_delta"]), 1) if eligible else 0.0
        return CraftActionEvaluation(action_type=str(config["action_type"]), action_name=str(config["action_name"]), target_mod=mod, eligibility=eligible, failure_reason=failure_reason, expected_cost=expected_cost, expected_value_delta=expected_value_delta, brick_risk=float(config["brick_risk"]), confidence_delta=round((probability * 0.12) - (float(config["brick_risk"]) * 0.08), 2), probability=round(probability, 2), expected_value_after_step=0.0, notes=notes)

    def _repair_step(self, opportunity: ScanOpportunity) -> CraftActionEvaluation | None:
        if not opportunity.risk_flags:
            return None
        if "fractured" in opportunity.risk_flags or "influenced" in opportunity.risk_flags:
            return CraftActionEvaluation(action_type="annul_repair", action_name="Annul / repair simple", target_mod="cleanup", eligibility=True, failure_reason="", expected_cost=18.2, expected_value_delta=12.0, brick_risk=0.2, confidence_delta=-0.04, probability=0.55, expected_value_after_step=0.0, notes="Passo defensivo para limpar ou estabilizar a base antes de investir pesado.")
        return None
    def _build_linking_step(self, opportunity: ScanOpportunity, target_link_count: int, value_delta: float) -> CraftActionEvaluation | None:
    ...
    def _build_exit_estimate(self, opportunity: ScanOpportunity, target: FlipTargetRecommendation, steps: List[CraftActionEvaluation], market_index: Dict[tuple[str, str, int, str], List[float]], budget: float, expected_craft_cost: float) -> ExitMarketEstimate | None:
        ...
        for link_count in link_options:
            prices = list(market_index.get(self._market_key(opportunity.item_family, opportunity.base_type, link_count, opportunity.ilvl), []))
            if link_count == self._link_count(opportunity) and not prices:
                prices = [price for price in (opportunity.market_floor, opportunity.market_median) if price > 0]
            if link_count != self._link_count(opportunity) and not prices:
                continue
            market_floor, market_median, market_spread, comparables_count = self._market_stats_from_prices(prices)
            if link_count == self._link_count(opportunity):
                ...
                comparables_count = max(comparables_count, opportunity.comparables_count)
    ...
    def _passes_stage_b(self, opportunity: ScanOpportunity, target: FlipTargetRecommendation, steps: List[CraftActionEvaluation], exit_estimate: ExitMarketEstimate, confidence_breakdown: PlanConfidenceBreakdown, expected_profit: float, budget: float) -> Tuple[bool, List[str]]:
        ...
        required_link_count = int(exit_estimate.required_structure.get("link_count", self._link_count(opportunity)))
        if required_link_count > self._link_count(opportunity):
            linking_steps = [step for step in steps if step.action_type == "socket_linking" and step.eligibility]
    ...
    def _mark_stop_point(self, opportunity: ScanOpportunity, steps: List[CraftActionEvaluation], expected_profit: float) -> None:
        cumulative_cost = 0.0
        for step in steps:
    ...
    def _build_stop_condition(self, opportunity: ScanOpportunity, steps: List[CraftActionEvaluation], expected_profit: float) -> str:
        cumulative_cost = 0.0
        for step in steps:
            cumulative_cost += step.expected_cost
            interim_profit = step.expected_value_after_step - opportunity.listed_price - cumulative_cost
    ...
    def _build_plan_explanation(self, opportunity: ScanOpportunity, target: FlipTargetRecommendation, steps: List[CraftActionEvaluation], exit_estimate: ExitMarketEstimate, confidence_breakdown: PlanConfidenceBreakdown) -> str:
        bits = [
        ...
        if opportunity.item_family == "body_armour_defense":
            bits.append(f"Estrutura: {self._defence_profile(opportunity)}/{self._attribute_profile(opportunity)}, {self._link_count(opportunity)}L atual.")
            if target.requires_suppression:
                bits.append("Suppression exigido e validado para base com componente de Evasion/Dex.")
            else:
                bits.append("Planner não força Spell Suppression porque a base não sustenta esse alvo ou o mercado não paga por isso.")
        required_links = exit_estimate.required_structure.get("link_count", self._link_count(opportunity))
    ...
    def _market_key(self, item_family: str, base_type: str, link_count: int, ilvl: int) -> tuple[str, str, int, str]:
        ilvl_band = "any"
    ...
    def _build_market_index(self, opportunities: List[ScanOpportunity]) -> Dict[tuple[str, str, int, str], List[float]]:
        index: Dict[tuple[str, str, int, str], List[float]] = {}
        for opportunity in opportunities:
            key = self._market_key(opportunity.item_family, opportunity.base_type, self._link_count(opportunity), opportunity.ilvl)
            index.setdefault(key, []).append(opportunity.listed_price)
        return index

    def _market_stats_from_prices(self, prices: List[float]) -> tuple[float, float, float, int]:
        filtered = sorted(float(price) for price in prices if price > 0)
        if not filtered:
            return (0.0, 0.0, 0.0, 0)
        floor = filtered[0]
        middle = len(filtered) // 2
        median = (filtered[middle - 1] + filtered[middle]) / 2 if len(filtered) % 2 == 0 else filtered[middle]
        spread = max(filtered) - min(filtered)
        return (round(floor, 1), round(median, 1), round(spread, 1), len(filtered))

    def _pricing_position(self, expected_sale_value: float, market_floor: float, market_median: float, market_spread: float) -> str:
        if market_floor <= 0 and market_median <= 0:
            return "near_market"
        if expected_sale_value <= max(market_floor * 1.05, market_floor + 5.0):
            return "below_floor"
        if market_median > 0 and expected_sale_value > max(market_median * 1.18, market_floor + max(market_spread, 15.0)):
            return "outlier"
        return "near_market"
    ...
    def _six_link_cost_chaos(self) -> float:
        fusing_rate = float(self.scanner.currency_rates.get("Orb of Fusing", 1.0) or 1.0)
    ...
    def _attribute_profile(self, opportunity: ScanOpportunity) -> str:
        ...
        mapping = {"armour": "str", "evasion": "dex", "energy_shield": "int", "armour_evasion": "str_dex", "armour_energy_shield": "str_int", "evasion_energy_shield": "dex_int"}
