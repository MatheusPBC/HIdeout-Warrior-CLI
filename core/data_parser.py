...
class RePoeParser:
    ...
    def _download_raw_data(self, url: str) -> Dict[str, Any]:
        ...
        print(
            f"📥 [RePoeParser] Baixando dados de: {url} (pode demorar alguns segundos)..."
        )
        try:
    ...
    def build_local_db(self, force_download: bool = False):
        ...
        if not force_download and os.path.exists(self.parsed_weights_path):
            print(
                "✅ [RePoeParser] Tabela de pesos O(1) já estruturada. Usando cache local de acesso."
            )
            return
        ...
        print(
            "⚙️  [RePoeParser] Compactando mais de 100 mil registros do Path of Exile em uma HashTable..."
        )
        parsed_db = {}
        ...
        for mod_id, mod_data in raw_mods.items():
            ...
            for gw in generation_weights:
                clean_weights.append({"tag": gw.get("tag"), "weight": gw.get("weight")})
        ...
        self.db = parsed_db
        print(
            f"📦 [RePoeParser] Compressão concluída! {len(self.db)} afixos chaveados para cálculos Probabilísticos."
        )
    ...
    def get_weight(self, mod_id: str) -> Optional[List[Dict[str, Any]]]:
        ...
        if not self.db:
            print(
                "⚠️ [RePoeParser] Banco de dados em memória vazio. Rodando auto-build..."
            )
            self.build_local_db()

        return self.db.get(mod_id, {}).get("weights")

    def get_weight_for_tag(self, mod_id: str, tag: str) -> int:
        """
        Retorna o peso específico de um mod para uma tag específica.
        Retorna 0 se o mod não existe ou não tem weight para essa tag.
        """
        weights = self.get_weight(mod_id)
        if not weights:
            return 0
        for w in weights:
            if w.get("tag") == tag:
                return w.get("weight", 0)
        return 0
